...
resource "aws_iam_role" "company-users-organization-administrators" {
    ...
    path = "/"
    description = "A role to delegate access for organization-administrators in the company-users AWS account to the company-test AWS account."
...
resource "aws_iam_role" "company-users-developers" {
    ...
    path = "/"
    description = "A role to delegate access for developers in the company-users AWS account to the company-test AWS account."
    assume_role_policy = <<EOF
{
    "Version": "2012-10-17",
    "Statement": [
